#!/usr/bin/env node
/**
 * Unit-test runner for the Plan module aggregation logic.
 *
 * The Plan aggregation (listRecurring / nextDue / habitCalendarMap /
 * timelineGroups / addPeriod) lives in src/plan.ts and is deliberately
 * React-Native-free so it can be unit-tested under plain Node — exactly like
 * the finance chart aggregation. This script:
 *   1. Transpiles every pure module + the test suite via ts.transpileModule
 *      (strips types only, no type checking, no module resolution) into a
 *      project-local `.tmptest` tree that preserves relative import structure,
 *      so `require('../plan')` keeps working.
...
const FILES = [
  ...
  ['src/plan.ts', 'plan.js'],
  ['src/import/__plan_tests.ts', 'import/__plan_tests.js'],
];
...
const SUITES = [
  ['Plan module aggregation (timeline / recurring / habit link)', 'import/__plan_tests.js'],
];
